...
for name, path in zip(data, data_paths):
    for df_type in ["full",
        ...
        #for mixin_pct in tqdm(list(range(0, 110, 20))[1:], desc="Mixin Percentage"):
            ...
            if not skip_transactionModel and not os.path.isfile("../working/{}_{}_{}_synthTransactionScoring_transactions.pkl".format(name, df_type, mixin_pct)):
                ...
                for sub_train_X, sub_train_y, sub_test_X, sub_test_y in zip(train_X, train_y, test_X, test_y):
                    ...
                    if name == "IBM-AML":
                        ...
                        synth_symple_size = int(sub_train_X.shape[0] * mixin_pct)
                        if synth_symple_size > train_X_synth.loc[(train_X_synth["To Bank"] != sub_name) & (train_X_synth["From Bank"] != sub_name)].shape[0]:
                            train_X_synth_mixin = train_X_synth.loc[(train_X_synth["To Bank"] != sub_name) & (train_X_synth["From Bank"] != sub_name)].sample(synth_symple_size, random_state=42, replace=True)
                        else:
                            train_X_synth_mixin = train_X_synth.loc[(train_X_synth["To Bank"] != sub_name) & (train_X_synth["From Bank"] != sub_name)].sample(synth_symple_size, random_state=42)

                    elif name == "IBM-CCF":
                        sub_name = sub_train_X["Bank"].value_counts().idxmax()
                        synth_symple_size = int(sub_train_X.shape[0] * mixin_pct)
                        if synth_symple_size > train_X_synth.loc[train_X_synth["Bank"] != sub_name].shape[0]:
                            train_X_synth_mixin = train_X_synth.loc[train_X_synth["Bank"] != sub_name].sample(synth_symple_size, random_state=42, replace=True)
                        else:
                            train_X_synth_mixin = train_X_synth.loc[train_X_synth["Bank"] != sub_name].sample(synth_symple_size, random_state=42)
                    else:
                        raise ValueError("Unknown dataset")
                ...
                with open("../working/{}_{}_{}_synthTransactionScoring_transactions.pkl".format(name, df_type, mixin_pct), "wb") as file:
                    pickle.dump({"overall": cls_report, "per_bank": results, "type": "transactions", "dataset": name, "mixin_percentage": mixin_pct, "synth_type": df_type}, file)

            # create transaction based model with Random Oversampling
            if not skip_transactionModelOS and not os.path.isfile("../working/{}_{}_{}_synthTransactionScoring_transactions_ROS_{}.pkl".format(name, df_type, mixin_pct, os_ratio)):
                ...
                with open("../working/{}_{}_{}_synthTransactionScoring_transactions_ROS_{}.pkl".format(name, df_type, mixin_pct, os_ratio), "wb") as file:
                    pickle.dump({"overall": cls_report, "per_bank": results, "type": "transactions over-sampled ({})".format(os_ratio), "dataset": name, "mixin_percentage": mixin_pct, "synth_type": df_type}, file)

        test_X = pd.concat(test_X)
        test_y = pd.concat(test_y)
        if not skip_transactionModel and not os.path.isfile("../working/synth/{}_{}_synth_synthTransactionScoring_transactions.pkl".format(name, df_type)):
            print("{} - {} - Purely Synth - Transaction based model".format(name, df_type))
            #clf = XGBClassifier(n_estimators=100, device="cuda", verbosity=2, random_state=42)
            clf = make_pipeline(StandardScaler(), GridSearchCV(XGBClassifier(random_state=42, device= "cuda"),
                                                               param_grid={"eta": [0.5, 0.1, 0.05],
                                                                           "min_child_weight": [0.5, 1, 4],
                                                               ...
                                                               n_jobs=-1,
                                                               verbose=1))
            clf.fit(train_X_synth, train_y_synth)
            test_pred = clf.predict(test_X)
            cls_report = classification_report(test_y, test_pred)
            cls_report += "roc-auc-score                        {0:.4f}  {0:.4f}".format(roc_auc_score(test_y, test_pred), test_y.shape[0])
            print(cls_report)
            with open("../working/synth/{}_{}_synth_synthTransactionScoring_transactions.pkl".format(name, df_type), "wb") as file:
                pickle.dump({"overall": cls_report, "type": "transactions", "dataset": name, "mixin_percentage": np.inf, "synth_type": df_type}, file)

        if not skip_transactionModelOS and not os.path.isfile("../working/synth/{}_{}_synth_synthTransactionScoring_transactions_ROS_{}.pkl".format(name, df_type, os_ratio)):
            print("{} - {} - Purely Synth - Transaction based model with Random Oversampling".format(name, df_type))
            if train_y_synth.value_counts().min() / train_y_synth.value_counts().max() < 0.2:
                ros = RandomOverSampler(sampling_strategy=os_ratio, random_state=42)
                ros_train_X_synth, ros_train_y_synth = ros.fit_resample(train_X_synth, train_y_synth)
            else:
                ros_train_X_synth, ros_train_y_synth = train_X_synth, train_y_synth
            #clf = XGBClassifier(n_estimators=100, device="cuda", verbosity=2, random_state=42)
            clf = make_pipeline(StandardScaler(), GridSearchCV(XGBClassifier(random_state=42, device= "cuda"),
                                                               param_grid={"eta": [0.5, 0.1, 0.05],
                                                                           "min_child_weight": [0.5, 1, 4],
                                                               ...
                                                               n_jobs=-1,
                                                               verbose=1))
            clf.fit(ros_train_X_synth, ros_train_y_synth)
            test_pred = clf.predict(test_X)
            cls_report = classification_report(test_y, test_pred)
            cls_report += "roc-auc-score                        {0:.4f}  {0:.4f}".format(roc_auc_score(test_y, test_pred), test_y.shape[0])
            print(cls_report)
            with open("../working/synth/{}_{}_synth_synthTransactionScoring_transactions_ROS_{}.pkl".format(name, df_type, os_ratio), "wb") as file:
                pickle.dump({"overall": cls_report, "type": "transactions over-sampled ({})".format(os_ratio), "dataset": name, "mixin_percentage": np.inf, "synth_type": df_type}, file)
