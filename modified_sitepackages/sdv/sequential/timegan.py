...
class TimeGan():
    ...
    def _train(self, data):
        ...
        for epoch in epoch_iterator:
            ...
            for X, T in train_dataloader:
                ...
                if self._wandb:
                    wandb.log({"Comb Supervisor Loss": self.err_d, "Comb Encoder & Decoder Loss": np.mean([x.detach().cpu().numpy() for x in enc_dec_avg])})
    ...
    def train_numpy(
        ...
        if internal_additional_attributes is not None:
            ...
            internal_additional_attributes = None
        ...
        if (internal_attributes is not None) and (internal_additional_attributes is not None):
        ...
        elif (internal_attributes is not None) and (internal_additional_attributes is None):
            internal_features_extdend = np.concatenate((internal_features,
                                                        np.repeat(np.expand_dims(internal_attributes, axis=1), internal_features.shape[1], axis=1)), axis=2)
        elif (internal_attributes is None) and (internal_additional_attributes is not None):
            internal_features_extdend = np.concatenate((internal_features,
                                                        np.repeat(np.expand_dims(internal_additional_attributes, axis=1), internal_features.shape[1], axis=1)), axis=2)
        else:
            internal_features_extdend = internal_features
...
class TIMEGANSynthesizer(LossValuesMixin, BaseSynthesizer):
    ...
    def __init__(self, metadata, max_sequence_len: int, enforce_min_max_values=True, enforce_rounding=False,
                 ...
                 epochs= 500000, batch_size= 16, cuda= "cuda", verbose= False, use_wandb= False):
        super().__init__(
            metadata=metadata,
